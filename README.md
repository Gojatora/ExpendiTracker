...
- **Packaging**: Standalone Android build via EAS Build (see `eas.json`)
...
## Setup (mobile — development via Expo Go)
...
The `preview` build profile in `eas.json` bakes in `EXPO_PUBLIC_API_URL` at build time (EAS cloud builds do not read your local `.env` file) — update that value in `eas.json` to match your current LAN IP before building. The resulting APK also requires `usesCleartextTraffic: true` (configured via the `expo-build-properties` plugin in `app.json`) to reach a plain-HTTP local backend — see Known Limitations below.
...
The mobile app has also been **packaged as a standalone Android APK** via EAS Build and verified on a physical device — registration, login, and expense creation all confirmed working end-to-end against the live backend.
...
- Two chart types (vs. Benchmark and vs. Previous Month) were rebuilt using plain React Native views instead of the `victory-native` charting library, after hitting four separate rendering inconsistencies with its `Bar`/`CartesianChart` components (overlapping bars, invisible axis labels, an unreliable `tickCount` fix, and labels that appeared/disappeared depending on unrelated screen state). Only the 12-month trend line chart still uses `victory-native`, which has been reliable throughout
